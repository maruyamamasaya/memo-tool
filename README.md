...
8. 初期表示が作成日の降順であること、ヘッダーの「タイトル」「作成日」「更新日」をクリックするたび昇順・降順が切り替わること、タイトルと作成日の期間で絞り込めることを確認します。
9. 別ブラウザでメモ・フォルダがリアルタイム同期し、非メンバーと別グループの操作が拒否されることを確認します。

### メモの編集方法

一覧は「タイトル」「タグ / フォルダ」「操作」の3列で、更新日時はタイトルの上に表示します。初期表示は作成日の降順です。ヘッダーの「タイトル」「作成日」「更新日」をクリックすると、それぞれを基準に昇順・降順を切り替えられます。一覧上部では、タイトルに含まれるワードと作成日の開始・終了日で絞り込めます。長いタイトルとサイドバーのフォルダ名は1行で省略されます。拡張子をクリックすると専用の設定画面が開き、フォルダ名をクリックするとそのフォルダへ移動します。PCではメモ行をサイドバーのフォルダまたは「未分類」へドラッグ＆ドロップして移動できます。行末の「⋯」からも設定画面やタグ、ダウンロードなどを操作できます。サイドバー右端はドラッグまたは矢印キーで幅を変更でき、その幅は端末に保存されます。
