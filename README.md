...
幅700px以下では一覧を1列にし、検索・フォルダ・タグ・ゴミ箱・設定をヘッダーの「メニュー」内へ格納します。新規作成は右下の丸い `＋` ボタンからいつでも行えます。実機では、メニューが横にはみ出さないこと、ダイアログをスクロールできること、ソフトウェアキーボード表示中も保存状態を確認できることを確認してください。
