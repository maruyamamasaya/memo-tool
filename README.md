...
3. タグの追加・削除・クリック絞り込み、フォルダの作成・改名・削除・メモ移動・未分類化を確認します。PCではメモ行をサイドバーのフォルダまたは「未分類」へドラッグ＆ドロップして移動できることも確認します。フォルダ名変更・削除はフォルダ横の「編集」から行います。
4. メモを開いて「最近」の順序と上部の最大5件が更新され、ゴミ箱のメモが除外されることを確認します。
5. ゴミ箱移動、復元、確認ダイアログ後の完全削除と削除日時順を確認します。
6. 設定から5種類のテーマを切り替え、背景、パネル、文字、アクセントの配色と再読込後の復元を確認します。
7. 通常メモ、チェックリスト、定型文をコピーし、チェックリストが `[ ]` / `[x]` 形式になることを確認します。
8. 初期表示が作成日の降順であること、「タイトル」のクリックで昇順・降順が切り替わること、日時メニューで「作成日時」「更新日時」を選べること、矢印で日時の昇順・降順を切り替えられることを確認します。検索欄横の歯車から詳細検索を開き、タイトルと作成日の期間で絞り込めることも確認します。
9. 別ブラウザでメモ・フォルダがリアルタイム同期し、非メンバーと別グループの操作が拒否されることを確認します。

### メモの編集方法

一覧は「タイトル」「タグ / フォルダ」「操作」の3列で、更新日時はタイトルの上に表示します。初期表示は作成日の降順です。ヘッダーの「タイトル」をクリックするとタイトルの昇順・降順を切り替えられます。横の日時メニューでは「作成日時」「更新日時」を選択でき、矢印ボタンで日時の昇順・降順を切り替えられます。検索欄横の歯車から「検索の詳細」を開くと、タイトルに含まれるワードと作成日の開始・終了日で絞り込めます。長いタイトルとサイドバーのフォルダ名は1行で省略されます。拡張子をクリックすると専用の設定画面が開き、フォルダ名をクリックするとそのフォルダへ移動します。PCではメモ行をサイドバーのフォルダまたは「未分類」へドラッグ＆ドロップして移動できます。行末の「⋯」からも設定画面やタグ、ダウンロードなどを操作できます。サイドバー右端はドラッグまたは矢印キーで幅を変更でき、その幅は端末に保存されます。

編集画面は1つの入力欄です。1行目をタイトル、最終行の `#タグ` をタグとして扱い、その間を本文として保存します。最終行で `#` を入力すると既存タグの候補が表示され、入力が止まってから1.5秒後に自動保存します。保存後も画面は閉じず、そのまま編集を続けられ、`Esc` キーで保存して一覧へ戻れます。

## スマートフォン表示

幅700px以下では一覧を1列にし、検索・フォルダ・タグ・ゴミ箱・設定をヘッダーの「メニュー」内へ格納します。メモの表示はモーダルレイヤーではなく、本文を読みやすい独立したページへ切り替わります。幅701px以上でマウス操作が可能なPCでは、メモを従来どおり新しいウインドウで表示します。新規作成は右下の丸い `＋` ボタンからいつでも行えます。実機では、メニューが横にはみ出さないこと、表示ページと編集画面をスクロールできること、ソフトウェアキーボード表示中も保存状態を確認できることを確認してください。
